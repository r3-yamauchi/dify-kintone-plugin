"""
# where: kintone_integration/tools/kintone_query_docs.py
# what: kintone クエリ構文ドキュメントをツール内に埋め込み、テキストとして返す。
# why: 外部ファイルに依存せず常にクエリ仕様を参照できるようにするため。
"""
...
# kintone クエリ言語の詳細ドキュメント（参考用.txt から移植）
KINTONE_QUERY_DOCUMENT = """
# kintoneクエリー記法ドキュメント
...
class KintoneQueryDocTool(Tool):
    """
    埋め込み済みの kintone クエリ記法ドキュメントを返すツール。
    """

    def _invoke(self, tool_parameters: Dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        document = KINTONE_QUERY_DOCUMENT.strip()
        if not document:
